This post is a follow-up to [Writing HTML sucks and No-code doesn't help](https://rogovoy.me/blog/writing-html-sucks) what I've posted two months ago.
...
I wanted to have a tool that worked directly with my code, allowing me to edit the UI inside the browser.
...
Imagine if you could change styles of any DOM element on the page using the Devtools, and it would be automatically saved to your code exactly the way you would do it yourself.

Impulse is that, and much more, but with a much better UX.

It's completelly local and open-source.
No SaaS.
No servers.
No accounts.
...
I feel like it's important to re-iterate on the problem Impulse is solving.
...
For one, it's just what I've been using for the last few years, and it also happens to be the most popular front-end framework at the time of creating Impulse.
...
It also allows you to _not_ come up with semantic names for styles which helps to tighten the feedback loop, which has been my goal in the first place.

Since using Tailwind is as easy as adding or removing classes, it naturally made a very easy CSS framework to intergrate with.

If you've haven't used Tailwind, this project might be a great reason try it out. A full-pledged support with auto-completion and other goodies is on its way. 
...
Use Impulse to add or remove classes from any DOM element and it would also edit your code.
...
Again, all of the changes are applied to the source code.

Oftentimes I'd catch myself spending 30 minutes streight creating new UI in the browser without looking at the code even once.
Then, when I do open the code, it's exactly the same code I'd write manually.
...
Impulse is just trying to remove the boring parts.

So, every time you do need to switch to your code editor, save yourself some brain cycles by using this feature to instantly jump to the place you are likely looking for.
